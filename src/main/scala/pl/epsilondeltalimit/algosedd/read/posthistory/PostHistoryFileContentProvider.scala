...
object PostHistoryFileContentProvider extends Transformation with Logging {
  ...
  override def apply(c: Catalog): Catalog =
    c.put {
      Dep.map2("postHistory")(c.get[SparkSession]("spark"), c.get[String]("pathToPostHistoryFile")) { (spark, pathToPostHistoryFile) =>
        import spark.implicits._

        logger.warn(s"Loading data from file: $pathToPostHistoryFile.")

        new XmlFileStorage(spark).readFromXmlFile("row", Schema, pathToPostHistoryFile)
