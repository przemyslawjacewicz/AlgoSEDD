...
object CommentsFileContentProvider extends Transformation with Logging {
  ...
  override def apply(c: Catalog): Catalog =
    c.put {
      Dep.map2("comments")(c.get[SparkSession]("spark"), c.get[String]("pathToCommentsFile")) { (spark, pathToCommentsFile) =>
        logger.warn(s"Loading data from file: $pathToCommentsFile.")

        new XmlFileStorage(spark).readFromXmlFile("row", Schema, pathToCommentsFile)
