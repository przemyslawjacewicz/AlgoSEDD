...
object PostLinksFileContentProvider extends Transformation with Logging {
  ...
  override def apply(c: Catalog): Catalog =
    c.put {
      Dep.map2("postLinks")(c.get[SparkSession]("spark"), c.get[String]("pathToPostLinksFile")) { (spark, pathToPostLinksFile) =>
        import spark.implicits._

        logger.warn(s"Loading data from file: $pathToPostLinksFile.")

        new XmlFileStorage(spark).readFromXmlFile("row", Schema, pathToPostLinksFile)
