...
object RelativePopularityByAggregationIntervalAndTag extends (Catalog => Catalog) with Logging {
  override def apply(c: Catalog): Catalog =
    c.put {
      c.get[DataFrame]("entriesCount")
        .map { entriesCount =>
          entriesCount
            .withColumn(
              "q__share",
              col("q__entries_count_for_aggregation_interval_and_tag") / col(
        ...
        .as("relativePopularityByAggregationIntervalAndTag")
    }
}
