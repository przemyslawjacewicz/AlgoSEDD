...
import .countEntriesByCreationDateAndTag
...
object DataEntriesCountByCreationDateAndTag extends Transformation with Logging {
  override def apply(c: Catalog): Catalog =
    c.put {
      ...
      c.get[DataFrame]("tagsByCreationDataFromQuestions").flatMap(tagsByCreationDataFromQuestions =>
        c.get[DataFrame]("tagsByCreationDataFromAnswers").flatMap(tagsByCreationDataFromAnswers =>
          c.get[DataFrame]("tagsByCreationDataFromComments").flatMap(tagsByCreationDataFromComments =>
            c.get[DataFrame]("tagsByCreationDataFromVotes").flatMap(tagsByCreationDataFromVotes =>
              c.get[DataFrame]("tagsByCreationDataFromPostHistory").flatMap(tagsByCreationDataFromPostHistory =>
                c.get[DataFrame]("tagsByCreationDataFromPostLinks").map(tagsByCreationDataFromPostLinks =>
